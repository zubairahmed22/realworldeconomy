  ...
  (function($) {
    ...
    $.extend(true,_R, {
      ...
      *************************************/	
      animateTheCaptions : function(nextli, opt,recalled,mtl) {		
        ...
        if (allcaptions)
          $.each(allcaptions,function(i) {
            var _nc = $(this);
            _nc.TweenLite.set(_nc.find('.tp-videoposter'),{autoAlpha:1});
            _nc.TweenLite.set(_nc.find('iframe'),{autoAlpha:0});
          })
        
        // COLLECT ALL CAPTIONS
        if (opt.layers[index])
          $.each(opt.layers[index], function(i,a) { allcaptions.push(a); });
        if (opt.layers["static"])
          $.each(opt.layers["static"], function(i,a) { allcaptions.push(a); });
        
        // GO THROUGH ALL CAPTIONS, AND MANAGE THEM
        if (allcaptions)
          $.each(allcaptions,function(i) {	
            _R.animateSingleCaption($(this),opt,base_offsetx,base_offsety,i,recalled)
          }); 
    
        var bt=$('body').find('#'+opt.c.attr('id')).find('.tp-bannertimer');
        bt.data('opt',opt);
      ...
      ***************************************/
      animateSingleCaption : function(_nc,opt,offsetx,offsety,i,recalled,triggerforce) {
        ...
        if (opt.debugMode) {
          ...
          _nc.on("hover, mouseenter",function() {
            var ltxt = "",
              spa = 0;
            if (_nc.data())
              $.each(_nc.data(),function(key,val) {
        ...
        _nc.find('.tp-resizeme, .tp-resizeme *').each(function() {
            calcCaptionResponsive($(this),opt,"rekursive",_responsive);
        });
        ...
        _nc.TweenLite.set(_pw,{zIndex:tpcapindex, top:calcy,left:calcx,overwrite:"auto"});
        ...
        if (triggerforce && _nc.data('timeline')!=undefined) {			
          ...
          _nc.TweenLite.killTweensOf(_nc);		
          _nc.unbind('hover');
        ...
        var $time = _nc.data('timeline') ? _nc.data('timeline').time() : 0,
          ...
          tl = _nc.data('timeline') || new _nc.TimelineLite({smoothChildTiming:true});
        ...
        if ($progress<1 && _nc.data('outstarted') != 1 || staticdirection==2 || triggerforce) {			
          ...
          if (_nc.data('splitin')!=undefined && _nc.data('splitin').match(/chars|words|lines/g) || _nc.data('splitout')!=undefined && _nc.data('splitout').match(/chars|words|lines/g)) {
            ...
            _nc.data('mySplitText',new _nc.SplitText(splittarget,{type:"lines,words,chars",charsClass:"tp-splitted",wordsClass:"tp-splitted",linesClass:"tp-splitted"}));					
          ...
          if (!_nc.data('$anims') || triggerforce || reverseanim) {				
            ...
            $result.anim.ease = $from.anim.ease = $from.anim.ease || _nc.Power1.easeInOut;
              ...
              _nc.hover(function(e) {
                 ...
                 if (intl && intl.progress()==1) {						 		
                   ...
                   if (nc.data('newhoveranim')===undefined || 	nc.data('newhoveranim')==="none")	{						 		
                     nc.data('newhoveranim',nc.TweenLite.to(nc,t.speed,t.anim));						 	
          ...
          if (staticdirection != 2 || triggerforce) {
            ...
            if (animobject != _nc) {	
              ...
              tl.add(_nc.TweenLite.set(_nc, $a.r.anim));
              $a.r = newAnimObject();	
            ...
            newtl.eventCallback("onStart",function(){			  	
              _nc.TweenLite.set(_nc,{visibility:"visible"});
            // FIX VISIBLE IFRAME BUG IN SAFARI
            if (_nc.data('iframes'))
              _nc.find('iframe').each(function() {
                _nc.TweenLite.set($(this),{autoAlpha:1});
              })
                _nc.TweenLite.set(_pw,{visibility:"visible"});
              var data={};
              data.layer = _nc;
            ...
            if ($mask_frm) {				  						  	
              ...
              tl.add(_nc.TweenLite.fromTo(_mw,$a.f.speed,$mask_frm.anim,$mask_rsl,$from.elemdelay),mdelay);				  	
            } else {
              tl.add(_nc.TweenLite.set(_mw,{overflow:"visible"},$from.elemdelay),0);				  	
            }		    				 
          }
          ...
          if (($progress === 0 || staticdirection==2) &&  $end!=="bytrigger" && !triggerforce && $end!="sliderleave") 				
            if (($end!=undefined) && (staticdirection==-1 || staticdirection==2) && ($end!=="bytriger")) 			
              _nc.TweenLite.delayedCall(parseInt(_nc.data('end'),0)/1000,_R.endMoveCaption,[_nc,_mw,_pw,opt]);
            else 
              _nc.TweenLite.delayedCall(999999,_R.endMoveCaption,[_nc,_mw,_pw,opt]);						
        ...
        _nc.TweenLite.set(_mw,{width:eow, height:eoh});
        if (_nc.data('loopanimation')=="on") _nc.TweenLite.set(_lw,{minWidth:eow,minHeight:eoh});
    
        if (_nc.data('slidelink')!=0 && (_nc.data('slidelink')==1 || _nc.hasClass("slidelink"))) {
          _nc.TweenLite.set(_mw,{width:"100%", height:"100%"});
          _nc.data('slidelink',1);
        } else {
          _nc.TweenLite.set(_mw,{width:"auto", height:"auto"});
      ...
      endMoveCaption : function(_nc,_mw,_pw,opt) {
        ...
        var tl = new _nc.TimelineLite(),
          subtl = new _nc.TimelineLite(),
          newmasktl = new _nc.TimelineLite(),				
        ...
        subtl.eventCallback("onComplete",function(){			  	
              _nc.TweenLite.set(_nc,{visibility:"hidden"});
              _nc.TweenLite.set(_pw,{visibility:"hidden"});
              var data={};
              data.layer = _nc;
      ...
      removeTheCaptions : function(actli,opt) {		
        ...
        if (opt.layers["static"])
          $.each(opt.layers["static"], function(i,a) { allcaptions.push(a); });
        removetime.TweenLite.killDelayedCallsTo(_R.endMoveCaption);
    ...
    **********************************************************************************************/
    ...
    var newAnimObject = function() {
      ...
      a.anim.rotation = 0;
      a.anim.ease = a.anim.Power3.easeOut;
      a.anim.force3D = "auto";
      a.speed = 0.3;
      a.anim.autoAlpha = 1;
    ...
    var getAnimDatas = function(frm,data,reversed) {		
      ...
      if (customarray)	
        $.each(customarray,function(index,pa) {
          var p = pa.split(":")
          var w = p[0],
            v = p[1];
    ...
    var makeArray = function(obj,opt,show) {
      ...
      if (!$.isArray(obj) && $.type(obj)==="string" && (obj.split(",").length>1 || obj.split("[").length>1)) {
        ...
        obj = new Array();
        if (newobj)
          $.each(newobj,function(index,element) {
    ...
    var calcCaptionResponsive = function(nc,opt,level,responsive) {
      ...
      if (!nc.hasClass("tp-splitted")) {
        ...
        nc.css("-webkit-transition", "none");
          ...
          if (hashover) nc.TweenLite.set(nc,obj.styleProps);
        ...
        if (level!="rekursive") {
          ...
          if (obj.color!="nopredefinedcolor") 				
            nc.TweenLite.set(nc,{color:obj.color,overwrite:"auto"});
    ...
    var callCaptionLoops = function(el,factor) {
      ...
      if (el.hasClass("rs-pendulum")) {			
        if (el.data('loop-timeline')==undefined) {
          el.data('loop-timeline',new el.TimelineLite);
          var startdeg = el.data('startdeg')==undefined ? -20 : el.data('startdeg'),
            enddeg = el.data('enddeg')==undefined ? 20 : el.data('enddeg'),
            speed = el.data('speed')==undefined ? 2 : el.data('speed'),
            origin = el.data('origin')==undefined ? "50% 50%" : el.data('origin'),
            easing = el.data('easing')==undefined ? el.Power2.easeInOut : el.data('ease');
    
            
          startdeg = startdeg * factor;
          enddeg = enddeg * factor;
    
          el.data('loop-timeline').append(new el.TweenLite.fromTo(el,speed,{force3D:"auto",rotation:startdeg,transformOrigin:origin},{rotation:enddeg,ease:easing}));
          el.data('loop-timeline').append(new el.TweenLite.fromTo(el,speed,{force3D:"auto",rotation:enddeg,transformOrigin:origin},{rotation:startdeg,ease:easing,onComplete:function() {
      ...
      if (el.hasClass("rs-rotate")) {			
        if (el.data('loop-timeline')==undefined) {
          ...
          var startdeg = el.data('startdeg')==undefined ? 0 : el.data('startdeg'),
            ...
            speed = el.data(`speed`)==undefined ? 2 : el.data(`speed`),
            origin = el.data('origin')==undefined ? "50% 50%" : el.data('origin'),
            easing = el.data('easing')==undefined ? el.Power2.easeInOut : el.data('easing');
    
          startdeg = startdeg * factor;
          enddeg = enddeg * factor;
    
          el.data('loop-timeline').append(new el.TweenLite.fromTo(el,speed,{force3D:"auto",rotation:startdeg,transformOrigin:origin},{rotation:enddeg,ease:easing,onComplete:function() {
            el.data('loop-timeline').restart();
          }}));
        }
      ...
      if (el.hasClass("rs-slideloop")) {			
        if (el.data('loop-timeline')==undefined) {
          ...
          el.data('loop-timeline').append(new el.TweenLite.fromTo(el,speed,{force3D:"auto",x:xs,y:ys},{x:xe,y:ye,ease:easing}));
          el.data('loop-timeline').append(new el.TweenLite.fromTo(el,speed,{force3D:"auto",x:xe,y:ye},{x:xs,y:ys,onComplete:function() {
            el.data('loop-timeline').restart();
          }}));
        }
      }
      ...
      if (el.hasClass("rs-pulse")) {			
        if (el.data('loop-timeline')==undefined) {
          ...
          el.data('loop-timeline').append(new el.TweenLite.fromTo(el,speed,{force3D:"auto",scale:zoomstart},{scale:zoomend,ease:easing}));
          el.data('loop-timeline').append(new el.TweenLite.fromTo(el,speed,{force3D:"auto",scale:zoomend},{scale:zoomstart,onComplete:function() {
            el.data('loop-timeline').restart();
          }}));
        }
      }
      ...
      if (el.hasClass("rs-wave")) {			
        if (el.data('loop-timeline')==undefined) {
          ...
          var  yo = (0-el.height()/2) + (radius*(-1+(parseInt(oo.y,0)/100))),
            ...
            el.data('loop-timeline').append(new el.TweenLite.fromTo(angobj,speed,
                          {	a:360	},
                          {	a:0,
                            force3D:"auto",
                            ease:el.Linear.easeNone,
                            onUpdate:function() {
    
                              var rad = angobj.a * (Math.PI / 180);
                                    el.TweenLite.to(angobj.element,0.1,{force3D:"auto",x:angobj.xoffset+Math.cos(rad) * angobj.unit, y:angobj.yoffset+angobj.unit * (1 - Math.sin(rad))});
